#!/usr/bin/env python3
"""
BBAC ICS Framework - Generate Plots
...
class GeneratePlots:
    ...
    def plot_ablation_comparison(
        ...
        metrics = {
            'Accuracy': [ablation_results[c]['metrics']['accuracy'] for c in configs],
            'F1': [ablation_results[c]['metrics']['f1_score'] for c in configs],
            'Latency': [ablation_results[c]['metrics']['latency']['mean'] for c in configs]
        }
        ...
        ax1.bar(x - width/2, metrics['Accuracy'], width, label='Accuracy', alpha=0.8)
        ax1.bar(x + width/2, metrics['F1'], width, label='F1 Score', alpha=0.8)
